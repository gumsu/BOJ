...
def cleaning_dust():
    ...
    while True:
        ...
        if nx < 0 or nx >= cleaner[1] or ny < 0 or ny >= c:
            up_air = (up_air + 1) % 4   # 방향 틀기
        else:
            if board[nx][ny] != 0:
                if up_row == cleaner[0] and up_col == 0:    # 공기 정화
                    board[nx][ny] = 0
                else:
                    board[up_row][up_col] = board[nx][ny]   # 반시계로 돌면서 한칸씩 땡겨온다. (문제와 역으로 처리)
                    board[nx][ny] = 0
            up_row = nx
            up_col = ny

    # 아랫 바람
    down_air = 2  # 아래부터 돈다(문제랑 역으로)
    down_row = cleaner[1]
    down_col = 0
    while True:
